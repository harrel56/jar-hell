...
import {formatBytecodeVersion, formatBytes, formatLicenseType, Gav, gavToString, ResolvedPackage} from '@/util.ts'
import {Link} from 'react-router-dom'

export const PackageCard = ({pkg}: { pkg: Gav | ResolvedPackage }) => {
  ...
  if ('effectiveValues' in pkg) {
    ...
    return (
      <Link to={`/packages/${gavToString(pkg)}`}>
        <Card
          className='min-w-[300px] max-w-[300px] flex flex-col justify-between
         cursor-pointer transition-all hover:brightness-[98%] dark:hover:brightness-[130%] hover:-translate-y-2'>
          <CardHeader>
            <CardDescription className='break-words'>{pkg.groupId}</CardDescription>
            <CardTitle className='break-words'>{pkg.artifactId}</CardTitle>
